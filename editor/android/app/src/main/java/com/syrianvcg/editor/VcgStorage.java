...
public class VcgStorage {
    ...
    public void saveFile(VcgFile file) {
        ...
        // استخدام transaction واحدة بدلاً من اثنتين منفصلتين لتجنب Race Condition
        filesPrefs.edit()
            .putString(toSave.getStorageKey(), toSave.getContent())
            .putString("folder::" + toSave.getStorageKey(), toSave.getFolderId())
            .apply();
    ...
    public void deleteFile(String projectId, String name) {
        String safeName = sanitizeForIndex(name);
        // transaction واحدة لضمان الحذف الذري
        filesPrefs.edit()
            .remove(projectId + "::" + safeName)
            .remove("folder::" + projectId + "::" + safeName)
            .apply();
