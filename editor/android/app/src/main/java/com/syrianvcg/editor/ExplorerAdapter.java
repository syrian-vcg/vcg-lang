...
public class ExplorerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ExplorerAdapter(List<VcgFolder> folders, List<VcgFile> files, Listener listener,
                           VcgStorage storage, String projectId) {
    ...
    /** عدد الملفات الفعلي داخل المجلد من التخزين — يتجنب الاعتماد على الـ list المحلية */
    private int countItemsIn(String folderId) {
        if (storage == null || projectId == null) return 0;
        return storage.getFilesInFolder(projectId, folderId).size();
    }
